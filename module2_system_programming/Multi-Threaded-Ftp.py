#!/usr/bin/env python
import threading import Queue import time from ftplib import FTP
# Thanks to SPSE-445 for the list :)
ftpHostList = [ 'ftp.x.org', 'ftp4.FreeBSD.org', 'ftp.ncsa.uiuc.edu', 
'ftp.mozilla.org', 'ftp.crans.org' ] class 
WorkerThread(threading.Thread) :
	...
	def run(self) :
		while True :
			...
			# login to ftp host anonymously and list the 
			# dirs
			try :
				conn = FTP(host)
				conn.login()
				...
				raise
			self.queue.task_done() queue = Queue.Queue() 
threads = [] for i in range(1, 10) :
